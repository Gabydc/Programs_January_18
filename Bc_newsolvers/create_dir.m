...
dir = '/mnt/sda2/cortes/Research/2018/articles/JCP_18/3D/';
if(use_wells)
    if(model_SPE)
        folder=['cp_' num2str(use_cp)  'SPE10_' num2str(numel(layers))  'DT_' num2str(DT/day), ...
            'step_' num2str(nstep) 'P_' num2str(P) 'tol_e-' num2str(tol) ];
    else
        folder=['cp_' num2str(use_cp) 'per_' num2str(per) 'sz_' num2str(nx)  'layers_', ...
            num2str(nz)  'DT_' num2str(DT/day) 'step_' num2str(nstep), ...
            'P_' num2str(P) 'tol_e-' num2str(tol) 'lay_dir_'  l_dir];
    end
else
    if(model_SPE)
        folder=[ 'cp_' num2str(use_cp) 'SPE10_' num2str(numel(layers))  'DT_' num2str(DT/day), ...
        ...
        folder=['cp_' num2str(use_cp) 'per_' num2str(per) 'sz_' num2str(nx) '_layers_', ...
            num2str(nz)  'DT_' num2str(DT/day) 'step_' num2str(nstep), ...
            'bc_' num2str(P_b) 'tol_e-' num2str(tol) 'lay_dir_'  l_dir];
    end
end
mkdir([dir], folder)
dir1 = [dir folder '/'];
