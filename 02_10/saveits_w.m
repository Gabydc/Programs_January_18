function saveits_w(dir1,file,use_ICCG,use_DICCG,use_POD,dpod,ts,dv,preport,last)
lp = length(dpod);
text = [dir1 file];
for i=1:ts
...
if ~exist(text, 'file')
    ...
    fprintf(fileID,'\\caption{Number of iterations. Injection through the left boundary.}\\label{table:liter1}   \n');
    fprintf(fileID,' \\begin{tabular}{lllllll}\n');
    fprintf(fileID,'\\hline\\noalign{\\smallskip} \n');
    fprintf(fileID,' $\\frac{\\kappa_2}{\\kappa_1}$ &d &Total& \\multicolumn{2}{c}{DICCG} &Total&\\% of \\\\ \n');
    fprintf(fileID,' &    &   & &+ DICCG&Iterations\\\\ \n');
        fprintf(fileID,'\\hline\n');
    fclose(fileID);
