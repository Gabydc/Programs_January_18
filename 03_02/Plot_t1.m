...
clim = [0 1];
if(model_SPE)
    subplotcbspei(nf,clim,k,Np,G,nz,time,Sat)
else
    subplotcbspei(nf,clim,k,Np,G,nz,time,Sat)
end
%%
...
file{nf} = ['4_Pressure'];
clim = [pmin pmax];
subplotcbspei(nf,clim,k,Np,G,nz,time,Pressure/barsa)
%%
T=1;
...
clear xy
for i = 1 : 4 
y(:,i) = Pressure(:,i)/barsa;
xy(i,1) = T;
 T = (i+1)*ceil(k/Np);
end

 dir ='/dev/media/Sphinx/Doctorado_Delft/2018/February/03_02/';
Plot_mesh_sub(y, G, 'dnames', {{'Premeability_1'}}, ...
    'o_ax', 1, 'l_cb', false, 'o_cb', 1, 'figure', 1, 'showgrid', 2, ...
