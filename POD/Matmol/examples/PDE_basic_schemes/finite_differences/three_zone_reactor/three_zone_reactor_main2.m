...
%...     
     [tout, yout] = ode15s(@three_zone_reactor_pdes2,t,x,options);
%...
     figure(1)
     plot(z1/L,100*yout(5:20,1:3:3*n1-2)./(yout(5:20,1:3:3*n1-2)+yout(5:20,2:3:3*n1-1)+cHin),'k');
     hold on
     plot(z2/L,100*yout(5:20,3*n1+1:4:3*n1+4*n2-3)./(yout(5:20,3*n1+1:4:3*n1+4*n2-3)+yout(5:20,3*n1+2:4:3*n1+4*n2-2)+cHin),'k');
     plot(z3/L,100*yout(5:20,3*n1+4*n2+1:3:3*n1+4*n2+3*n3-2)./(yout(5:20,3*n1+4*n2+1:3:3*n1+4*n2+3*n3-2)+yout(5:20,3*n1+4*n2+2:3:3*n1+4*n2+3*n3-1)+cHin),'k');
     xlabel('z');
     ylabel('xB(z,t)');
     title('Benzene concentration profiles inside the reactor')
%...
     figure(2)
     plot(z1/L,1e4*yout(5:20,2:3:3*n1-1)./(yout(5:20,1:3:3*n1-2)+yout(5:20,2:3:3*n1-1)+cHin),'k');
     hold on
     plot(z2/L,1e4*yout(5:20,3*n1+2:4:3*n1+4*n2-2)./(yout(5:20,3*n1+1:4:3*n1+4*n2-3)+yout(5:20,3*n1+2:4:3*n1+4*n2-2)+cHin),'k');
     plot(z3/L,1e4*yout(5:20,3*n1+4*n2+2:3:3*n1+4*n2+3*n3-1)./(yout(5:20,3*n1+4*n2+1:3:3*n1+4*n2+3*n3-2)+yout(5:20,3*n1+4*n2+2:3:3*n1+4*n2+3*n3-1)+cHin),'k');
     xlabel('z');
     ylabel('xT(z,t)');
     title('Thiophene concentration profiles inside the reactor')
%...
     figure(3)
     record = avifile('temperature2.avi')
     for k = 5:20,
       ...
       axis([0 1 20 180]);
       mov(k-4) = getframe(3)
       record = addframe(record,mov(k-4))
     end
     record = close(record)

%      plot(z1/L,yout(5:20,3:3:3*n1)-273.16,'k');
%      hold on
%      plot(z2/L,yout(5:20,3*n1+3:4:3*n1+4*n2-1)-273.16,'k');
%      plot(z3/L,yout(5:20,3*n1+4*n2+3:3:3*n1+4*n2+3*n3)-273.16,'k');
%      xlabel('z');
%      ylabel('T(z,t)');
%     title('Temperature profiles inside the reactor')
%...
     figure(4)
     record = avifile('activity.avi')
     for k = 5:20,
       plot(z2/L,yout(k,3*n1+4:4:3*n1+4*n2),'k');
       hold on
       xlabel('z');
       ylabel('\theta_A(z,t)');
       title('Catalyst activity')
       axis([zL1/L zL2/L 0 1.1]);
       mov(k-4)=getframe(4)
       record = addframe(record,mov(k-4))
     end
     record = close(record)

%      plot(z2/L,yout(5:20,3*n1+4:4:3*n1+4*n2),'k');
%      xlabel('z');
%      ylabel('theta(z,t)');
%     title('Catalyst activity')
%...
