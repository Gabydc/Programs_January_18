%... Updated by Filip Logist 11/04/2006
%... phi=max(0, min([4*r .75*r+.25 2]));

function [fz]=smart_slope_limiter_fz(z,n,t,x,flux,dflux_dx)
%...
%...  The MatMol Group (2009)
%...
%...  function smart_slope_limiter returns the first derivative, fz, of a 
%...  flux function f(x) over the spatial domain z0 < z < zL from smart 
...
%...  limited by the slope limiter phi given in the smart case by
%...
%...  phi = max(0, min(4*r, 0.75*r+0.25, 2)) (updated by Filip Logist)
%...
...
%...      computation of r(2) and phi(2)
          if abs(x(2)-x(3)) < delta
              ...
              phi(2)=max(0, min([4*r(2) .75*r(2)+.25 2]));
          end
%...
%...      computation of r(3) and phi(3)
          if abs(x(3)-x(4)) < delta
              ...
              phi(3)=max(0, min([4*r(3) .75*r(3)+.25 2]));
          end
%...
...
%...          computation of r(i) and phi(i)
              if abs(x(i)-x(i-1)) < delta
                  ...
                  phi(i)=max(0, min([4*r(i) .75*r(i)+.25 2]));
              end
%...
%...          computation of r(i-1) and phi(i-1)
              if abs(x(i-1)-x(i-2)) < delta
                  ...
                  phi(i-1)=max(0, min([4*r(i-1) .75*r(i-1)+.25 2]));
              end
%...
...
%...      computation of r(n-1) and phi(n-1)
          if abs(x(n-1)-x(n-2)) < delta
              ...
              phi(n-1)=max(0, min([4*r(n-1) .75*r(n-1)+.25 2]));
          end
%...
%...      computation of r(n-2) and phi(n-2)
          if abs(x(n-2)-x(n-3)) < delta
              ...
              phi(n-2)=max(0, min([4*r(n-2) .75*r(n-2)+.25 2]));
          end
%...
