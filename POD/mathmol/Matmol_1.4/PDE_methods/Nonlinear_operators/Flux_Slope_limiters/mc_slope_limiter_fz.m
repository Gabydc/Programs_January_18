      function [fz]=mc_slope_limiter_fz(z,n,t,x,flux,dflux_dx)
%...
%...  The MatMol Group (2009)
%...
%...  function mc_slope_limiter returns the first derivative, fz, of a 
%...  flux function f(x) over the spatial domain z0 < z < zL from mc 
...
%...  limited by the slope limiter phi given in the mc case by
%...
%...  phi = max(0, min(2*r, (1+r)/2,2))
%...
...
%...      computation of r(2) and phi(2)
          if abs(x(2)-x(3)) < delta
              ...
              phi(2)=max(0, min([2*r(2) (1+r(2))/2 2]));
          end
%...
%...      computation of r(3) and phi(3)
          if abs(x(3)-x(4)) < delta
              ...
              phi(3)=max(0, min([2*r(3) (1+r(3))/2 2]));
          end
%...
...
%...          computation of r(i-1) and phi(i-1)
              if abs(x(i-1)-x(i-2)) < delta
                  ...
                  phi(i-1)=max(0, min([2*r(i-1) (1+r(i-1))/2 2]));
              end
%...
...
%...      computation of r(n-1) and phi(n-1)
          if abs(x(n-1)-x(n-2)) < delta
              ...
              phi(n-1)=max(0, min([2*r(n-1) (1+r(n-1))/2 2]));
          end
%...
%...      computation of r(n-2) and phi(n-2)
          if abs(x(n-2)-x(n-3)) < delta
              ...
              phi(n-2)=max(0, min([2*r(n-2) (1+r(n-2))/2 2]));
          end
%...
