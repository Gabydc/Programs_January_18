      function [fz]=vanleer_slope_limiter_fz(z,n,t,x,flux,dflux_dx)
%...
%...  The MatMol Group (2009)
%...
%...  function vanleer_slope_limiter returns the first derivative, fz, of a 
...
%...          computation of r(i-1) and phi(i-1)
              if abs(x(i-1)-x(i-2)) < delta
                  ...
                  phi(i-1)=(r(i-1)+abs(r(i-1)))/(1+r(i-1));
              end
%...
...
%...      computation of r(n-1) and phi(n-1)
          if abs(x(n-1)-x(n-2)) < delta
              ...
              phi(n-1)=(r(n-1)+abs(r(n-1)))/(1+r(n-1));
          end
%...
%...      computation of r(n-2) and phi(n-2)
          if abs(x(n-2)-x(n-3)) < delta
              ...
              phi(n-2)=(r(n-2)+abs(r(n-2)))/(1+r(n-2));
          end
%...
